...
@Tag(
        name = "REST APIs for customers in YY Bank",
        description = "REST APIs in YY Bank to FETCH customer details"
)
@RestController
...
public class CustomerController {
    ...
    public ResponseEntity<CustomerDetailsDto> fetchCustomerDetails(
            @RequestHeader("yyBank-correlation-id") String correlationId,
            @RequestParam @Pattern(regexp = "(^$|[0-9]{10})", message = "Mobile number must be 10 digits")
            String mobileNumber) {
        // Log
        logger.debug("yyBank-correlation-id found:{}", correlationId);
